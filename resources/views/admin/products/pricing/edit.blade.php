...
@section('title', "Edit Pricing Plan — {$product->name}")
...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Pricing Plan</h2>
    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $product->name }}</p>
</div>

<form action="{{ route('admin.products.pricing.update', [$product, $plan]) }}" method="POST" class="max-w-2xl">
    @csrf @method('PUT')
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-gray-900 dark:border-gray-800 p-6 space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="original_price" id="original_price" value="{{ old('original_price', $plan->original_price) }}"
            ...
            <div>
                ...
                <select name="billing_cycle" id="billing_cycle" required
                    ...
                    @foreach(['monthly', 'yearly', 'lifetime', 'one_time', 'usage', 'credits', 'free', 'trial', 'custom'] as $cycle)
                        <option value="{{ $cycle }}" {{ old('billing_cycle', $plan->billing_cycle) === $cycle ? 'selected' : '' }}>{{ ucwords(str_replace('_', ' ', $cycle)) }}</option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                ...
                placeholder="What's included in this plan">{{ old('description', $plan->description) }}</textarea>
    ...
    <div class="mt-6 flex items-center gap-3">
        <button type="submit" class="px-5 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors">Update Plan</button>
